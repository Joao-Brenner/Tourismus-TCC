...
class OverpassRequisicaoAlvo
{
    
    public static function requisicaoOverpassAlvo(
        int $osmId,
        string $osmType
    ): string {
        ...
        $query = "[out:json][timeout:25][maxsize:134217728];
                  $osmType($osmId);
                  out tags center;";
        ...
        error_log("cURL request headers OverpassAlvo: " . $requestHeaders);
        ...
        if ($resposta === false || $httpCode !== 200) {
            ...
            return json_encode([
                'erro_tecnico' => "Falha na comunicação com o Overpass ($httpCode)"
            ]);
        }
